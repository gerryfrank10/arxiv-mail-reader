...
export function parseArxivEmail(
  ...
  while (i < lines.length) {
    ...
    if (trimmed === '\\\\' && i + 1 < lines.length && lines[i + 1].trim().startsWith('arXiv:')) {
      ...
      // Date line (new submissions): "Date: Sat, 14 Feb 2026 20:46:47 GMT   (559kb)"
      let date = '';
      let size = '';
      if (i < lines.length && lines[i].trim().startsWith('Date:')) {
        ...
        // Date/size embedded in the arXiv line for replacements
        const revMatch = arxivLine.match(/replaced with revised version\s+(.+?)\s+\((\d+\w+)\)/);
        if (revMatch) { date = revMatch[1].trim(); size = revMatch[2]; }
      }

      // Skip blank line
      if (i < lines.length && lines[i].trim() === '') i++;

      // Parse metadata fields — stop at \\ abstract separator OR URL line
      let title = '';
      let authors = '';
      let categories: string[] = [];
      let comments = '';

      while (i < lines.length && lines[i].trim() !== '\\\\' && !isUrlLine(lines[i])) {
        const line = lines[i];
      ...
      // Skip the \\ abstract separator if present (not present for replacements)
      if (i < lines.length && lines[i].trim() === '\\\\') i++;
      ...
      const abstract = abstractLines.map(l => l.trim()).filter(Boolean).join(' ').trim();
      ...
      papers.push({
        ...
        arxivId, date, size,
        title: decodedTitle, authors: decodedAuthors, authorList: decodedList,
        categories, comments, abstract, url,
        pdfUrl: `https://arxiv.org/pdf/${arxivId}`,
        emailId, digestSubject, digestDate,
      });
    }
